...
function HTMLElements(string) {
	...
	for (let i = 0; i < htmlFindStringArray.length; i++) {
		if (htmlFindStringArray[i].startsWith('</')) {
			closingTags.push(htmlFindStringArray[i]);
		} else {
			openingTags.push(htmlFindStringArray[i]);
		}
	}
...
console.log(HTMLElements('<div>abc</div><p><em><i>test test test</i></em></b>'));
